...
As documented you should create/import your Celery app in ``contabile/__init__.py`` so that you
can run the worker via::

    python celery -A contabile worker

Additionally you will need to configure the project settings for Celery::

    # contabile.settings.staging.py
    import os
    from contabile.settings.base import *

    # Other settings would be here
    BROKER_URL = 'amqp://contabile_staging:%(BROKER_PASSWORD)s@%(BROKER_HOST)s/contabile_staging' % os.environ

You will also need to add the ``BROKER_URL`` to the ``contabile.settings.production`` so
...
RabbitMQ user will be named contabile_<environment> and the vhost will be named contabile_<environment>
